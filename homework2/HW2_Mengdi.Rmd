...
From the following regression output, this regression has 41 coefficients in total, excluding the intercept. Its R^2^ is 1-7186.9/13003.4 = 0.4473061, indicating that about 44.73% of the variability in the logarithm of the price is explain by the model. Out of the 43 coefficients, 36 are significant under a False Discovery Rate (FDR) of 10%.
...
1-7186.9/13003.4
```
After removing the insignificant variables ("ETRANS", "NUNITS", "BEDRMS") and re-estimate the model, the R^2^ is 1-7188.3/13003.4 = 0.4471984. There is only a very small decrease in R^2^, suggesting that the removed variables contributed little to explaining the variability in log-price.
...
pricey_drop1 <- glm(log(LPRICE) ~ . - AMMORT - ETRANS - NUNITS - BEDRMS, data=homes)
summary(pricey_drop1)
1-7188.3/13003.4 # not dropping "STATE"

# re-estimate the model
pricey_drop2 <- glm(log(LPRICE) ~ . - AMMORT - ETRANS - NUNITS - BEDRMS - STATE, data=homes)
# summary(pricey_drop2)
1-7493.4/13003.4 # dropping "STATE"
```
...
# regress log(PRICE) on everything except AMMORT 
pricey_subset <- glm(log(LPRICE) ~ .-AMMORT - gt20dwn, data=homes[subset,])
